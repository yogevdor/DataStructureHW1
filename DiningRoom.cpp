...
StatusType DiningRoom::addTable(int tableId, int capacity) { //DOR

}

StatusType DiningRoom::removeTable(int tableId) { //DOR

}
StatusType DiningRoom::enterDiningRoom(int guestId, int tableId, Guests_Tree& guests) { //YAARA
...
StatusType DiningRoom::leaveDiningRoom(int guestId, int tableId) { //YAARA
    if(guestId <= 0 || tableId <= 0)
        return StatusType::INVALID_INPUT;
    DiningRoom::Dining_Room_Val* current_table = this->tree.find(tableId);
    if(current_table == nullptr) //no such table
...
StatusType DiningRoom::reheatFood() { //YAARA
    this->mealCount++;
    return StatusType::SUCCESS;
}
StatusType DiningRoom::joinTables(int tableId1, int tableId2) { //YAARA
    ...
    try {
        AVLtree<Guests_Tree::Guest_Val*>* merged_tables = AVLtree<Guests_Tree::Guest_Val*>::mergeTrees(&(table_1->guestsTree), &(table_2->guestsTree));
...
int DiningRoom::getLastMeal() const{ //YAARA
